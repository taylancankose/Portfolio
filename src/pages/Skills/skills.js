export const Skills = {
  frontend: [
    {
      id: 1,
      name: "Javascript",
      level: "Intermediate",
      imgURL:
        "https://upload.wikimedia.org/wikipedia/commons/thumb/9/99/Unofficial_JavaScript_logo_2.svg/1200px-Unofficial_JavaScript_logo_2.svg.png",
    },
    {
      id: 2,
      name: "React",
      level: "Intermediate",
      imgURL: "https://avatars.githubusercontent.com/u/6412038?s=280&v=4",
    },
    {
      id: 3,
      name: "React Native",
      level: "Intermediate",
      imgURL: "https://avatars.githubusercontent.com/u/6412038?s=280&v=4",
    },
    {
      id: 4,
      name: "Redux",
      level: "Intermediate",
      imgURL:
        "https://media.licdn.com/dms/image/D5612AQH7_svwKSMIqA/article-cover_image-shrink_720_1280/0/1674827212376?e=2147483647&v=beta&t=yz7gBCl1j2xG2Rwsb59t9iB3ScZNx7MrJs6LMGw8cdk",
    },
    {
      id: 5,
      name: "Typescript",
      level: "Intermediate",
      imgURL:
        "https://upload.wikimedia.org/wikipedia/commons/thumb/4/4c/Typescript_logo_2020.svg/2048px-Typescript_logo_2020.svg.png",
    },
    {
      id: 6,
      name: "Figma",
      level: "Intermediate",
      imgURL:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTbdKGM9SVZXipbU9rJRscgDMrD3ku4Lu7ILb5_rt-Q1Q&s",
    },
  ],
  rest: [
    {
      id: 8,
      name: "NodeJS",
      level: "Basic",
      imgURL:
        "https://images.g2crowd.com/uploads/product/image/large_detail/large_detail_f0b606abb6d19089febc9faeeba5bc05/nodejs-development-services.png",
    },
    {
      id: 7,
      name: "GraphQL",
      level: "Basic",
      imgURL:
        "https://miro.medium.com/v2/resize:fit:400/1*oyUZ4k1clyKiaahg0pfixQ.png",
    },
    {
      id: 9,
      name: "MongoDB",
      level: "Basic",
      imgURL:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTNyOe4xJhJFvDUbm1OSFnnEc4plFTvdYrBmOfNf-YUNA&s",
    },
    {
      id: 12,
      name: "Agile\nMethodology",
      imgURL:
        "https://img.freepik.com/premium-vector/agile-development-method-vector-icon_116137-2066.jpg",
    },
    {
      id: 10,
      name: `Mobile\nMarketing`,
      imgURL: "https://pngimg.com/d/meta_PNG2.png",
    },
    {
      id: 13,
      name: `Problem\nSolving`,
      imgURL:
        "https://static.vecteezy.com/system/resources/thumbnails/015/561/376/small/idea-solution-illustration-on-a-background-premium-quality-symbols-icons-for-concept-and-graphic-design-vector.jpg",
    },
  ],
};

/*
Certificates & Education
Full Stack Web Development (workintech)
The Mobile App Path with React Native (patika.dev)
Advanced Frontend Web Development Path (patika.dev)
Backend Path with NodeJS (patika.dev)
React Front to Back (Udemy)
Frontend Developer (React) (Hackerrank)
React Native Using WebSockets: Live Stock Prices Mobile App (Udemy)
ASO Course by MobileAction (Mobile Action Growth University)
Mobile App User Acquisition Course by Tinuiti (Mobile Action Growth University)
Startup Wise Guys  Growth Circuit Accelerator Program
ING Inovation Center Accelerator Program
Fundamentals of Entrepreneurship (Draper University, Silicon Valley)
*/
